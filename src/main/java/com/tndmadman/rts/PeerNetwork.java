...
final class PeerNetwork implements CommandSink {
    ...
    private void send(String message, InetAddress address, int port) { try { PacketChunks.send(socket, message, address, port); } catch (IOException ex) { if (running) System.err.println("Send failed: " + ex.getMessage()); } }
    private void listenLoop() { byte[] buf = new byte[65535]; while (running) { DatagramPacket p = new DatagramPacket(buf, buf.length); try { socket.receive(p); String raw = new String(p.getData(), p.getOffset(), p.getLength(), StandardCharsets.UTF_8); String message = PacketChunks.receive(raw, p.getAddress(), p.getPort()); if (message != null) inbox.add(new NetPacket(message, p.getAddress(), p.getPort())); } catch (SocketTimeoutException ignored) { } catch (Exception ex) { if (running) System.err.println("UDP failed: " + ex.getMessage()); } } }
