...
final class GamePanel extends JPanel implements KeyListener, MouseListener, MouseMotionListener, MouseWheelListener, FocusListener {
    ...
    private final Object gameSettings; // resolved reflectively so HUD can use live bindings without hard dependency
    ...
    GamePanel(World world, GameFrame owner, PeerNetwork network, boolean devMode, PeerNetwork devAuthorityNetwork) {
        ...
        this.gameSettings = resolveGameSettings(owner);
    ...
    @Override protected void paintComponent(Graphics g) {
        ...
        if (settings.isShowFps() || (devMode && perfOverlayVisible)) {
    ...
    perfOverlay.draw(
            ...
            updateLabel(),
            perfStats.snapshot(),
            devMode && perfOverlayVisible ? networkStats : null,
            devMode && perfOverlayVisible ? hostStats : null,
            devMode && perfOverlayVisible
    );
}
        g2.dispose();
    ...
    private void drawHud(Graphics2D g2) {
        ...
        g2.drawString(
                "StarChem | " + PlayerRegistry.name(PlayerRegistry.localId())
                        + " | System: " + world.activeSystemId()
                        + " | Selected: " + world.selectedCount()
                        + dev,
                28, 36
        );
...
g2.drawString(
    "Galaxy: " + settings.bindingText("galaxy_map")
    + " | Inventory: " + settings.bindingText("inventory")
    + " | Narration: " + settings.bindingText("narration")
    + " | Formation: " + formation.label
    + " (" + settings.bindingText("formation") + ")"
    + " | Miner ranges: " + minerRanges
    + " (" + settings.bindingText("miner_range") + ")"
    + " | Audio: " + audio
    + " (" + settings.bindingText("mute_audio") + ")"
    + perf,
    28,
    102
);
...
g2.drawString(
    ...
    @Override public void mousePressed(MouseEvent e) {
        if (gameMenuVisible) {
            GameMenuOverlay.Action action = gameMenu.click(e.getX(), e.getY());
            switch (action) {
                case RESUME -> {
    ...
    @Override public void keyPressed(KeyEvent e) {
        if (settings.matches("mute_audio", e)) { boolean muted = ProceduralAudio.toggleMute(); world.status = muted ? "Audio muted." : "Audio enabled."; repaint(); return; }
        ...
        if (galaxyMapOpen) return;
        if (devMode && settings.matches("ai_debug_overlay", e)) { AiDevSettings.overlay = !AiDevSettings.overlay; world.status = "AI debug overlay: " + (AiDevSettings.overlay ? "ON" : "OFF") + "."; repaint(); return; }
        if (devMode && settings.matches("performance_overlay", e)) { perfOverlayVisible = !perfOverlayVisible; world.status = "Performance overlay: " + (perfOverlayVisible ? "ON" : "OFF") + "."; repaint(); return; }
        if (settings.matches("formation", e)) { formation = formation.next(); world.status = "Fleet formation: " + formation.label + "."; ProceduralAudio.play(SoundCue.SELECT); return; }
        if (settings.matches("miner_range", e)) { UnitRenderer.toggleMiningRangeOverlay(); world.status = "Miner range overlay: " + (UnitRenderer.miningRangeOverlayVisible() ? "ON" : "OFF") + "."; ProceduralAudio.play(SoundCue.SELECT); repaint(); return; }
        ...
        if (settings.matches("hold", e)) { issueSelectedOrder(UnitOrderType.HOLD, 0, 0, 0, 0, ""); clearCommandMode(); return; }
    ...
    private Object resolveGameSettings(GameFrame owner) {
        if (owner == null) return null;
        try {
            Method getter = owner.getClass().getMethod("gameSettings");
    ...
    private String hudBinding(String fallback, String... ids) {
        Object settings = this.gameSettings;
        if (settings != null) {
            for (String id : ids) {
                String value = readBindingText(settings, id);
    ...
    private String readBindingText(Object settings, String id) {
        try {
            Method m = settings.getClass().getMethod("bindingText", String.class);
            Object value = m.invoke(settings, id);
