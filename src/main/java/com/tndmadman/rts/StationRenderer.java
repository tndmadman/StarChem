...
final class StationRenderer {
    ...
    private static final double FAR_LOD_SCALE = 0.55;
    ...
    static void drawAtTime(Graphics2D source, Base base, Color playerColor, double time, double scale) {
        ...
        // Authored silhouettes remain unique/readable at strategic zoom while skipping moving
        // gantries, trusses, windows and other micro-detail whose projected size is sub-pixel.
        // This is presentation-only LOD: interaction bounds and station simulation are untouched.
    ...
    private static void drawRadar(Graphics2D g, Base base, Color owner, double time, int tier, boolean detailed) {
        ...
        dish.drawLine(-ring / 2 > Integer.MIN_VALUE ? (int)-ring / 2 : 0, 0, (int)ring / 2, 0);
        dish.setColor(EDGE);
    ...
    private static void drawDecoy(Graphics2D g, Base base, Color owner, double time, boolean detailed) {
        ...
        switch (profile) {
            case "shipyard" -> drawShipyard(g, base, muted(owner), time, detailed);
            case "manufacturing" -> drawManufacturing(g, base, muted(owner), time, detailed);
            case "laboratory" -> drawLaboratory(g, base, muted(owner), time, detailed);
            case RadarTowerRules.TIER_ONE -> drawRadar(g, base, muted(owner), time, 1, detailed);
            case RadarTowerRules.TIER_TWO -> drawRadar(g, base, muted(owner), time, 2, detailed);
            case RadarTowerRules.TIER_THREE -> drawRadar(g, base, muted(owner), time, 3, detailed);
            default -> drawOutpost(g, base, muted(owner), time, detailed);
        }
